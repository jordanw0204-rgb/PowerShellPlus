...
Click the **globe** beside the Windows Terminal button in the title bar. PowerShellPlus starts an embedded website only on this PC's active private IPv4 LAN addresses and shows the address plus a new eight-digit pairing code. Open that address on a phone connected to the same Wi-Fi/Ethernet network, enter the code, and every current terminal appears with live VT colors, scrollback, session names, grid/focus layouts, and font controls. Sessions added or removed on the PC are synchronized automatically.

Phone access is read-only by default. Enable **Allow paired phones to type in terminals** only when you need it. Remote keystrokes then go directly to the selected existing ConPTY, so PowerShell, Codex, SSH, and native programs keep exactly the permissions they already have; the browser does not create or resume a second shell. The phone never resizes the shared ConPTY, so its layout cannot disturb the desktop terminal.

This first release deliberately supports trusted LANs only and uses local HTTP. Its traffic is not encrypted, so do not use it on hotel, airport, guest, school, or other untrusted Wi-Fi. If Windows Firewall asks, allow **Private networks** only. Pairing sessions and codes are held in memory and invalidated when sharing stops. Requests must come from the same IPv4 subnet, WebSocket origins are checked, messages and connections are bounded, and direct public/router-forwarded traffic is rejected. Do not port-forward the LAN Remote port.
